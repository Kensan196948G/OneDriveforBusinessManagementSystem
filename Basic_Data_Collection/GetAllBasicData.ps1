...
# GetAllBasicData.ps1 - すべての基本データ収集スクリプト
...
try {
    if ($isAdmin) {
        ...
        foreach ($user in $allUsers) {
            ...
            Write-Progress -Activity "ユーザー情報とOneDriveクォータ情報を取得中" -Status "$processedUsers / $totalUsers ユーザー処理中 ($percentComplete%)" -PercentComplete $percentComplete
        ...
        Write-Progress -Activity "ユーザー情報とOneDriveクォータ情報を取得中" -Completed
        Write-Log "ユーザー情報とOneDriveクォータ情報の取得が完了しました。取得件数: $($userList.Count)" "SUCCESS"
    } else {
        # 一般ユーザーまたはゲストの場合、自分自身の情報のみ取得
        Write-Log "自分自身のユーザー情報とOneDriveクォータ情報を取得しています..." "INFO"
        ...
        Write-Log "ユーザー情報とOneDriveクォータ情報の取得が完了しました。取得件数: 1" "SUCCESS"
    }
} catch {
...
# JavaScript ファイルの生成
$jsContent = @"
// AllBasicData データ操作用 JavaScript
...
function exportTableToCSV() {
    ...
    downloadLink.download = 'AllBasicData_Export.csv';
...
function colorizeRows() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        var statusCell = rows[i].querySelector('td:nth-child(11)'); // 状態のセル
