...
// OneDrive対応状況でフィルタリングする関数
function filterByOneDrive(status) {
    var table = document.getElementById('errorTable');
    var rows = table.getElementsByTagName('tbody')[0].getElementsByTagName('tr');
    
    for (var i = 0; i < rows.length; i++) {
        var statusCell = rows[i].getElementsByTagName('td')[3]; // OneDrive対応列
...
// 詳細対応状況でフィルタリングする関数
function filterByOneDriveDetail(detail) {
    var table = document.getElementById('errorTable');
    var rows = table.getElementsByTagName('tbody')[0].getElementsByTagName('tr');
    
    for (var i = 0; i < rows.length; i++) {
        var detailCell = rows[i].getElementsByTagName('td')[10]; // 推奨対応列
        if (detail === "" || detailCell.textContent.includes(detail)) {
            rows[i].style.display = "";
        } else {
            rows[i].style.display = "none";
        }
...
function initFilters() {
    ...
    var onedriveFilter = document.querySelector('.onedrive-filter');
    var detailFilter = document.querySelector('.onedrive-detail-filter');
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        var detailCell = rows[i].getElementsByTagName('td')[10];
        
        if (userCell.textContent && userCell.textContent !== "N/A") {
            users.add(userCell.textContent);
        }
        if (statusCell.textContent) {
            statuses.add(statusCell.textContent);
        }
        if (detailCell.textContent) {
            if (detailCell.textContent.includes("アクセス可能")) {
                details.add("アクセス可能");
            } else if (detailCell.textContent.includes("アクセス不可")) {
                details.add("アクセス不可");
            } else {
                details.add("未確認");
            }
    ...
    // OneDrive対応状況をフィルターに追加
    Array.from(statuses).sort().forEach(function(status) {
        var option = document.createElement('option');
        option.value = status;
        option.textContent = status;
        onedriveFilter.appendChild(option);
    });
    
    // 詳細対応状況をフィルターに追加
    Array.from(details).sort().forEach(function(detail) {
        var option = document.createElement('option');
        option.value = detail;
